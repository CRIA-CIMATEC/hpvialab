...
if __name__ == '__main__':
    ...
    parser.add_argument("-pren", "--num-pretext-experiments", type=int, default=10,
    ...
    parser.add_argument("-rp", "--run-prediction", action="store_true",
                        help="it runs predictions of a dataset specified by the user with the \
                            'sample_name' of the 'constants.py' file")
    args = parser.parse_args()

    if not args.skip_pretext_hyperparams:
        c.use_ray = True
        # pt.train_pretext_tune_task(num_samples=args.num_pretext_experiments)
        ptoc.train_pretext_tune_task(num_samples=args.num_pretext_experiments, max_num_epochs=100)
    if args.run_single_augmentation:
        c.use_ray = False
        ptoc.train_pretext_full_config(ptoc.good_params_for_single_run, aug.AugmentationTypes.ORIGINAL, use_tune=False)
    ...
    if not args.skip_finetune_hyperparams:
        c.use_ray = True
        ftt.train_finetune_tune_task(data.DataSets.WESAD, 'TskCrrct', num_samples=args.num_finetune_experiments)
    c.use_ray = False
    if args.run_single_finetuning:
        ftt.finetune_to_target_full_config(ftt.good_params_for_single_run, checkpoint_dir=None, target_dataset=data.DataSets.WESAD, target_id='TskCrrct')
    if args.run_example_classification:
        re.run_example(data.DataSets.WESAD, 'task_head_0')
    if args.run_prediction:
        re.run_prediction(data.DataSets.GENERIC, 'task_head_0')
