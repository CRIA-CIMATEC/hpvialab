...
from mag_utils import load_train_data, mkdir, imread, save_images
...
class MagNet3Frames(object):
    ...
    def inference(self, frameA, frameB, amplification_factor):
        ...
        in_frames = [load_train_data([frameA, frameB, frameB],
                     ...
                     is_numpy=isinstance(frameA, np.ndarray))]
...
def deepmag_model(config_spec, config_file, phase, frame_shape):
	...
	setproctitle.setproctitle('{}_{}_{}' \
							  .format(phase, network_type, exp_name))
	...
	model.setup_for_inference(checkpoint, frame_shape[0], frame_shape[1])
	
	def run_frame_mode(prev_frame, frame, amplification_factor):
		return model.inference(prev_frame, frame, amplification_factor)[0]

	if phase == 'run':
		return run_frame_mode
	elif phase == 'run_temporal':
		raise NotImplementedError
	else:
		raise ValueError('Invalid phase argument. '
						'Expected ["run", "run_temporal"], '
						'got ' + phase)