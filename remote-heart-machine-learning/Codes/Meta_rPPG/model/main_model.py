...
import os
import itertools
from .sub_model import rPPG_Estimator, Convolutional_Encoder, Synthetic_Gradient_Generator
from .loss import ordLoss
import pickle
...
class meta_rPPG(nn.Module):
	...
	def __init__(self, opt, isTrain, continue_train=False, norm_layer=nn.BatchNorm2d):
		...
		self.loss_name = ['t_ordloss']
		self.visual_names = ['ground_truth', 'predicted']
	...
	def set_input_for_test(self, input):
		""" Method to separate input for test
      	Parameters:
      	input -- Stored parameters that will be separated to other variables
      	"""
	...
	def fewshot_test(self, epoch):
		...
		for i in range(self.opt.fewshots):
			optim.zero_grad()
			inter = A(self.input[0].unsqueeze(0).to(self.device))
		...
		with torch.no_grad():
			...
			data = self.input[1:]
			self.predict_array = []
			for i in range(0, len(data) - self.h.shape[1], self.h.shape[1]):
				inter = A(data[i:i+self.h.shape[1]].to(self.device))
				self.decision, self.predict = self.B_net(inter)
				self.predict_array.append(self.predict)
				self.B_net.feed_hc([tmp_h, tmp_c])
			self.predict_array = torch.cat(self.predict_array, axis = 0)
			self.predict = self.predict_array
	...
	def pretrain_theta_and_phi(self):
		"""Pretrain phase"""
	...
	def init_weights(net1, net2, init_type='normal', init_gain=0.02):
    	...
    	Parameters:
    	    net1 (network)   -- network to be initialized
         	net2 (network)   -- network to be initialized
	...
	def get_current_results(self, istest):
		if istest:
			gt = []
			if self.true_rPPG != []:
				gt = self.true_rPPG.cpu().clone()
			return {self.visual_names[1]: self.decision.cpu().clone(), self.visual_names[0] : gt}
		else:
	...
	def load_networks(self, suffix):
		...
		model_dict = torch.load(load_path1, map_location=self.device)
